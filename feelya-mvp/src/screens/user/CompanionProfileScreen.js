...
import { useApp, MOCK_COMPANIONS, generateAvailability } from '../../store/AppContext';
...
export default function CompanionProfileScreen({ navigation, route }) {
  ...
  const companion = MOCK_COMPANIONS.find((g) => g.id === guideId);
  ...
  if (!companion) {
    return (
      <Screen>
        <Header title="Companion" onBack={() => navigation.goBack()} />
        <View style={s.empty}>
          <Text style={s.emptyText}>Companion not found</Text>
        </View>
      </Screen>
    );
  }
  ...
  const confirmBooking = () => {
    ...
    if (!slot) return;
    const price = +(30 * companion.ratePerMin).toFixed(2);
    ...
    if (bookingId) {
      ...
      dispatch({
        ...
        payload: {
          id: 'b_' + Date.now(),
          guideId: companion.id,
          guideName: companion.name,
          date: slot.date,
          hour: slot.hour,
          ...
          price,
          status: 'upcoming',
          topics: companion.topics.slice(0, 2),
        },
      });
    }
    ...
    Alert.alert(
      ...
      `Chat with ${companion.name} on ${formatDate(slot.date)} at ${slot.label}`,
  ...
  return (
    <Screen>
      <Header title={companion.name} onBack={() => navigation.goBack()} />
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.profile}>
          ...
          <View style={s.ratingRow}>
            ...
            <Text style={s.ratingText}>{companion.rating} ({companion.conversations} conversations)</Text>
          </View>
        </View>

        {/* Bio */}
        <Card style={s.bioCard}>
          <Text style={s.bio}>{companion.bio}</Text>
        </Card>
        ...
        <View style={s.badgeRow}>
          {companion.badges.map((b) => (
            <Badge key={b} label={b} icon="ribbon-outline" />
          ))}
          {companion.verified && <Badge label="Verified" color={colors.success} icon="checkmark-circle-outline" />}
        </View>
      ...
      <BottomSheet visible={showConfirm} onClose={() => setShowConfirm(false)} title="Confirm Chat">
        <View style={s.confirmRow}>
          <Avatar name={companion.name} size={40} />
          <View style={{ marginLeft: 12, flex: 1 }}>
            <Text style={s.confirmName}>{companion.name}</Text>
        ...
        <View style={s.confirmDetails}>
          ...
          <View style={s.confirmDetailRow}>
            ...
            <Text style={s.confirmDetailValue}>${companion.ratePerMin.toFixed(2)}/min</Text>
          </View>
          <View style={s.confirmDetailRow}>
            <Text style={s.confirmDetailLabel}>Estimated total</Text>
            <Text style={[s.confirmDetailValue, { fontWeight: '700' }]}>
              ${(30 * companion.ratePerMin).toFixed(2)}
            </Text>
          </View>
        </View>
      ...
      <BottomSheet visible={showBooking && !showConfirm} onClose={() => setShowBooking(false)} title="Plan a chat">
        <Text style={s.bookingSubtitle}>Pick a time to chat with {companion.name}</Text>
