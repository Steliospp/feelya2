...
export default function GuideProfileScreen({ navigation, route }) {
  ...
  const { guideId, bookingId } = route.params || {};
  ...
  const confirmBooking = () => {
    ...
    Alert.alert(
      bookingId ? 'Rescheduled' : 'Booked',
      `Session with ${guide.name} on ${formatDate(slot.date)} at ${slot.label}`,
  ...
  return (
    <Screen>
      ...
      {selectedSlot && (
        <View style={s.footer}>
          <PrimaryButton title={bookingId ? 'Reschedule' : 'Book Session'} onPress={handleBook} />
        </View>
      )}

      <BottomSheet visible={showConfirm} onClose={() => setShowConfirm(false)} title="Confirm Booking">
        ...
        <PrimaryButton title="Confirm Booking" onPress={confirmBooking} style={{ marginTop: spacing.md }} />
      </BottomSheet>
    </Screen>
  );
}
